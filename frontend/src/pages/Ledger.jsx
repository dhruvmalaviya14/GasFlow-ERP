...
export default function Ledger() {
  ...
  // 2. Fetch specific or combined Ledger history populated with entries
  const { data: ledgerData, isLoading: ledgerLoading } = useQuery({
    queryKey: ['ledger', selectedHotelId],
    queryFn: async () => {
      const url = selectedHotelId ? `/ledger/${selectedHotelId}` : '/ledger';
  ...
  const handleDownloadPDF = () => {
    ...
    // Direct link to download invoice PDF - axiosInstance automatically injects Authorization token!
    axios({
      url: `/ledger/bill/download/${selectedHotelId}`,
      method: 'GET',
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            ...
            <div className="glass-panel p-5 rounded-2xl border border-slate-200/50 dark:border-slate-800/40">
              ...
              <h4 className="text-xl font-extrabold text-slate-800 dark:text-white mt-1">
                {selectedHotelId ? `Rs. ${ledgerData.hotel.rate} / Cylinder` : 'Variable (Per Hotel)'}
              </h4>
            </div>
          ...
          <div className="glass-panel rounded-3xl p-6 md:p-8 border border-slate-200/50 dark:border-slate-800/40">
            ...
            <div className="flex items-center gap-2.5 mb-6">
              ...
              <h3 className="text-lg font-bold text-slate-850 dark:text-white">
                {selectedHotelId ? `${ledgerData.hotel.name} Ledger Audits` : 'All Restaurants Combined Ledger Audits'}
              </h3>
            </div>
                ...
                <table className="w-full text-left border-collapse text-sm">
                  <thead>
                    <tr className="border-b border-slate-200/60 dark:border-slate-800/50 text-slate-400 text-xs uppercase font-semibold">
                      ...
                      {!selectedHotelId && <th className="pb-3">Hotel / Restaurant</th>}
                      ...
                      {selectedHotelId && <th className="pb-3 text-right">Outstanding Balance</th>}
                    </tr>
                  </thead>
